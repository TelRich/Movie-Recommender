...
movie_md['genres'] = movie_md.loc[:,'genres'].apply(lambda x: [genre['name'] for genre in ast.literal_eval(x)])
movie_cr['cast'] = movie_cr.loc[:,'cast'].apply(lambda x: [i['name'] for i in ast.literal_eval(x)])
movie_kw['keywords'] = movie_kw.loc[:,'keywords'].apply(lambda x: [i['name'] for i in ast.literal_eval(x)])

# change date data type
movie_md['release_date'] = pd.to_datetime(movie_md.loc[:,'release_date'], errors='coerce')
# dropped invalid id
...
count_matrix = count.fit_transform(merged_data['content'])
